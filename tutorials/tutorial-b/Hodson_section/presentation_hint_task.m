...
for t = 1:T % loop over time points  
            ...
            lnAo = zeros(size(state_posterior{factor})); % initialise matrix containing the log likelihood of observations
                for tau = 1:T % loop over tau
                    ...
                    if tau<t+1 % Collect an observation from the generative process when tau <= t
                        lnAo = get_likelihood(lnAo, NumModalities, a, outcomes, tau, state_posterior, NumFactors);
                    end
                    % 'forwards' and 'backwards' messages at each tau
                    if tau == 1 % first tau
                        lnD = %______? forward message
    ...
    for policy = 1:NumPolicies
        ...
        for timestep = t:horizon
            ...
            epistemic = G_epistemic_value(a(:),Expected_states(:));
            ...
            for modality = 1:NumModalities
                ...
                pragmatic = predictive_observations_posterior'*(C{modality}(:,t));
                Gintermediate(policy) = %_______________?
    ...
    for ni = 1:Ni 
        ...
        policy_posteriors(:,t) = %__________?
...
function lnAo = get_likelihood(lnAo, NumModalities, a, outcomes, tau, state_posterior, NumFactors)
